...
#else
	ndev = alloc_netdev(sizeof(struct netdev_priv_instance), "qca-nss-tstamp", NET_NAME_PREDICTABLE, nss_tstamp_ndev_setup);
#endif
	if (!ndev) {
...
nss_tx_status_t nss_tstamp_tx_buf(struct nss_ctx_instance *nss_ctx, struct sk_buff *skb, uint32_t if_num)
{
	...
	hdr_sz = (uint32_t)skb->data - (uint32_t)align_data;
