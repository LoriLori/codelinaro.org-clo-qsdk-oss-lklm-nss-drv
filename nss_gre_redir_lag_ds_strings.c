 ...
 */
struct nss_stats_info nss_gre_redir_lag_ds_stats_str[NSS_GRE_REDIR_LAG_DS_STATS_MAX] = {
...
/*
 * nss_gre_redir_lag_ds_stats_str_strings_read()
 *	Read gre_redir_lag_ds statistics names
 */
static ssize_t nss_gre_redir_lag_ds_stats_str_strings_read(struct file *fp, char __user *ubuf, size_t sz, loff_t *ppos)
{
	return nss_strings_print(ubuf, sz, ppos, nss_gre_redir_lag_ds_stats_str, NSS_GRE_REDIR_LAG_DS_STATS_MAX);
}

/*
 * nss_gre_redir_lag_ds_stats_str_strings_ops
 */
NSS_STRINGS_DECLARE_FILE_OPERATIONS(gre_redir_lag_ds_stats_str);
...
{
	nss_strings_create_dentry("gre_redir_lag_ds_stats_str", &nss_gre_redir_lag_ds_stats_str_strings_ops);
}
